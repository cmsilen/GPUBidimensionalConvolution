...
__global__ void bidimensionalConvolution(const uint8_t* __restrict__ imgs, const uint8_t* __restrict__ blurMap, uint8_t* results, uint16_t nBlocks, uint16_t layersNum) {
    ...
    // divisione a livello di blocco
    uint64_t basePixels = (ROWS_MATRIX * COLUMNS_MATRIX * layersNum) / nBlocks;
    uint64_t extraPixels = (ROWS_MATRIX * COLUMNS_MATRIX * layersNum) % nBlocks;

    uint64_t start, end;

    if (blockIdx.x < extraPixels) {
        start = blockIdx.x * (basePixels + 1);
        end = start + basePixels + 1;
    } else {
        start = blockIdx.x * basePixels + extraPixels;
        end = start + basePixels;
    }

    for(uint64_t j = start + threadIdx.x; j < end; j += blockDim.x) {
...
#else
    char filename[100] = "resultsV5/testTPB_";
#endif
