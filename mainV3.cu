...
float experiment(uint16_t nBlocks) {
    ...
    // Lancio del kernel
    bidimensionalConvolution<<<nBlocks, THREADS_PER_BLOCK>>>(d_imgs, d_blurMap, d_results, nBlocks, LAYERS_NUM, d_filters);
...
#else
    char filename[100] = "resultsV3/testTPB_";
#endif
